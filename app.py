...
/* ===== FINAL FIELD-SURFACE + CHECKBOX ALIGNMENT FIX ===== */
/* The checkbox control sits a few pixels lower than its one-line label in
   Streamlit 1.58. Move only the square upward; leave the text untouched. */
div[data-testid="stCheckbox"] label {
    display: flex !important;
    align-items: center !important;
    gap: 0.55rem !important;
...
/* Make every form field fully opaque again.  Apply the solid surface to
   both Streamlit/BaseWeb wrappers and the editable element so the gym
   background cannot show through on any page. */
...
/* Solid closed select/multiselect fields everywhere. */
div[data-testid="stSelectbox"] div[data-baseweb="select"] > div,
div[data-testid="stMultiSelect"] div[data-baseweb="select"] > div {
    background-color: #FFFCF7 !important;
    opacity: 1 !important;
...
/* Solid open dropdown panels/options too. */
div[data-baseweb="popover"],
div[data-baseweb="popover"] [role="listbox"],
div[role="listbox"],
div[data-baseweb="popover"] [role="option"],
...
/* ===== DEFINITIVE CHECKBOX ALIGNMENT FIX =====
   Streamlit 1.58 renders the visible checkbox inside the first span.
   Move ONLY that indicator upward; do not move the text wrapper. */
div[data-testid="stCheckbox"] label > span:first-child,
div[data-testid="stCheckbox"] label span:has(input[type="checkbox"]) {
    position: relative !important;
    top: -3px !important;
    transform: none !important;
    margin: 0 !important;
...
/* Keep the warning text in its normal position. */
...
/* ===== ABSOLUTE FINAL SOLID-FIELD OVERRIDES =====
   Force all editable/select surfaces to be truly opaque. */
...
div[data-testid="stTextInput"] input:not([type="checkbox"]):not([type="radio"]),
div[data-testid="stNumberInput"] input,
...
/* Open select menus are solid as well. */
div[data-baseweb="popover"],
